...
// Backend API only. Do NOT put ThingsBoard credentials in frontend.
const LIVE_API_URL =
  (window.location.protocol === "http:" || window.location.protocol === "https:")
    ? `${window.location.protocol}//${window.location.hostname}:3000/api/tb/live`
    : "http://localhost:3000/api/tb/live";

// Map ThingsBoard device UUIDs to frontend space IDs
const DEVICE_TO_SPACE_ID = {
  "f947bef0-4194-11f1-9a3c-1fb54f58cf69": "assl",
  "d8591950-422f-11f1-bfc4-25a208a27468": "abacws"
};
...
// =======================
async function loadLiveSpacesFromBackend() {
  try {
    const res = await fetch(LIVE_API_URL, { method: "GET" });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);

    const json = await res.json();
    if (!json?.ok || !json?.data) throw new Error("Invalid live payload");

    // Supports two backend shapes:
    // 1) { ok: true, data: { temperature, humidity, occupancy, noiseLevel } } -> updates assl
    // 2) { ok: true, data: { spaces: [{ id, temperature, humidity, occupancy, noiseLevel }] } } -> updates many
    if (Array.isArray(json.data?.spaces)) {
      json.data.spaces.forEach((incoming) => {
        // Map backend device UUID to frontend space ID
        const spaceId = DEVICE_TO_SPACE_ID[incoming.id] || incoming.id;
        const targetSpace = spaces.find((s) => s.id === spaceId);
        if (!targetSpace) return;

        const t = toNum(incoming.temperature);
        const h = toNum(incoming.humidity);
        const o = toNum(incoming.occupancy);
        const n = toNum(incoming.noiseLevel);

        if (t !== null) targetSpace.tempC = t;
        if (h !== null) targetSpace.humidity = Math.round(h);
        if (o !== null) targetSpace.occupied = clampInt(o, 0, targetSpace.capacity);
        if (n !== null) targetSpace.noiseDb = clampInt(n, 28, 100);

        targetSpace.camera.online = true;
        targetSpace.camera.lastSeenSec = 0;
        targetSpace.camera.confidence = 0.95;

        const r = occupancyRatio(targetSpace);
        targetSpace.history = [...targetSpace.history.slice(1), clampInt(Math.round(r * 100), 0, 100)];
      });
    } else {
      const t = toNum(json.data.temperature);
      const h = toNum(json.data.humidity);
      const o = toNum(json.data.occupancy);
      const n = toNum(json.data.noiseLevel);
    ...
    // Fail quietly; simulation fallback keeps UI usable.
    dataSource = "client-demo";
    tbWarning = `Live API unavailable (${e?.message || "unknown error"})`;
  }
}
...
function simulateUpdate() {
  spaces.forEach(s => {
    // Keep live space stable when backend data is connected
...
function bindEvents() {
  ...
  $("#refresh").addEventListener("click", async () => {
    await loadLiveSpacesFromBackend();
    simulateUpdate();
    toast(
      "Live update",
      dataSource === "thingsboard"
        ? "Updated from backend live feed."
        : (tbWarning || "Backend unavailable; using simulated data.")
    );
    render();
  });
