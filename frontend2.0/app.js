    ...
    for (const device of TB_DEVICES) {
      try {
        ...
        if (targetSpace) {
          ...
          targetSpace.camera.lastUpdateTs = Date.now();
...
const USERS_KEY = "study_sense_users_v1";
const SESSION_KEY = "study_sense_session_v1";
...
function getLastSeenSec(space) {
  if (space.camera && typeof space.camera.lastUpdateTs === "number") {
    return Math.max(0, Math.floor((Date.now() - space.camera.lastUpdateTs) / 1000));
  }
  return Number.isFinite(space.camera?.lastSeenSec) ? space.camera.lastSeenSec : 0;
}
...
function setAuthMode(mode) {
  ...
  authSwitch.textContent = isRegister ? "Already have an account? Login" : "No account? Register";
  confirmLabel.hidden = !isRegister;
  authConfirm.hidden = !isRegister;
...
function updateUserChip() {
  userName.textContent = currentUser ? currentUser.username : "Guest";
...
function registerUser(username, password, confirmPassword) {
  ...
  if (exists) return "This username already exists.";
...
function renderCard(s, { mode, index }) {
  ...
  const lastSeenSec = getLastSeenSec(s);
      ...
      <div class="metric">
        ...
        <div class="cam">
          ...
          <div class="badge" title="Last update from camera/sensor feed">
            <span class="dot ${dotClassFor("cam", s.camera.online)}"></span>${s.camera.online ? `${lastSeenSec}s ago` : `stale`}
          </div>
        </div>
      </div>
...
function openDrawer(space) {
  ...
  const lastSeenSec = getLastSeenSec(space);
  ...
  drawerBody.innerHTML = `
    <div class="kpi">
      ...
      <div class="box"><div class="label"><span>Data freshness</span><span>Source</span></div><div class="value">${space.camera.online ? `${lastSeenSec}s ago` : `Stale`} • Camera</div></div>
    ...
    <div class="row">
      ...
      <hr class="sep"/>
      <p>
        <b>Last update:</b> ${space.camera.online ? `${lastSeenSec}s ago` : "offline"}<br/>
        <b>Estimated occupancy:</b> ${space.occupied} people<br/>
        <b>Noise:</b> ${capLabel(noiseC)}<br/>
...
function bindEvents() {
  logoutBtn.addEventListener("click", () => {
    logout();
    toast("Logged out", "Please log in again to access dashboard.");
  });
