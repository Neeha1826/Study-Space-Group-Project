...
function hasTbConfig() {
  return Boolean(TB_BASE_URL && TB_DEVICE_ID && TB_USERNAME && TB_PASSWORD);
}
...
async function fetchTelemetryWithToken(token) {
  const keys = "temperature,humidity,occupancy,noiseLevel";
  const url = `${TB_BASE_URL}/api/plugins/telemetry/DEVICE/${TB_DEVICE_ID}/values/timeseries?keys=${keys}`;
...
async function loadDeviceTelemetry() {
  ...
  let res = await fetchTelemetryWithToken(token);

  if (res.status === 401) {
    tbJwtToken = null;
    const newToken = await getTbToken();
    res = await fetchTelemetryWithToken(newToken);
  }

  if (!res.ok) {
    throw new Error(`TB telemetry failed (${res.status})`);
  }
